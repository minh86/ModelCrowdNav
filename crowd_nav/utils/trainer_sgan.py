...
class Trainer_SGAN(object):
    def __init__(self, dataset_root, device=None, dataset="cacheFiles", obs_len=8, num_iterations=10000, num_epochs=50):
        """
    ...
    def run_train(self):
        parser = self.sgan_parser()
        args = parser.parse_args(["--dataset_root", self.dataset_root,
                                  "--dataset_name", self.dataset,
                                  "--output_dir", self.dataset_root,
                                  "--obs_len", str(self.obs_len),
                                  "--num_iterations", str(self.num_iterations),
                                  "--num_epochs", str(self.num_epochs)
                                  ])
        return self.main(args)

    def sgan_parser(self):
    ...
    def main(self, args):
        ...
        train_path = get_dset_path(args.dataset_name, 'train', args.dataset_root)

        long_dtype, float_dtype = get_dtypes(args, self.device)
        ...
        # Save last model
        pbar.close()
        generator_path = os.path.join(args.output_dir, 'generator.pt')
        # logging.info('Saving generator...')
        # torch.save(generator.state_dict(), generator_path)
        return np.average(checkpoint['G_losses']['G_total_loss']), self.generator
