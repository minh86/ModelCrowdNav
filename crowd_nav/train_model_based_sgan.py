...
parser.add_argument('--world_model', type=str, default='sgan')
...
os.makedirs(os.path.join(args.output_dir, 'cacheFiles/test'))
cacheFile_val = os.path.join(args.output_dir, 'cacheFiles/val/')
cacheFile_train = os.path.join(args.output_dir, 'cacheFiles/train/')
cacheFile_test = os.path.join(args.output_dir, 'cacheFiles/test/')
cacheFile_generate = os.path.join(args.output_dir, 'cacheFiles/generate.txt')
...
if not args.use_dataset:
    ...
    test_raw_memory, _ = GetRealData(dataset_file=test_datapath, phase="test", stride=stride, windows_size=windows_size,
                                     dataset_slice=test_sl, Store_for_world_fn=Store_for_world_fn, cacheFile=cacheFile_test)
