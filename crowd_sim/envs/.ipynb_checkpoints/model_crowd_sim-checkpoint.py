...
class ModelCrowdSim(gym.Env):
    ...
    def step(self, action, update=True):
        ...
        new_pos = self.sim_world (current_s)[0]
        new_pos = torch.reshape(new_pos,(self.human_num,2)).tolist()
        if update:
            ...
            for i, _ in enumerate(self.humans):
                self.humans[i].px, self.humans[i].py, self.humans[i].vx, self.humans[i].vy = \
                    ...
                    (new_pos[i][0] - self.humans[i].px)/self.humans[i].time_step, \
                    (new_pos[i][1] - self.humans[i].py)/self.humans[i].time_step
            self.global_time += self.time_step            
            ...
            for i in range(self.human_num):
                h = ObservableState(new_pos[i][0], new_pos[i][1], \
                    (new_pos[i][0] - self.humans[i].px)/self.humans[i].time_step, \
                    (new_pos[i][1] - self.humans[i].py)/self.humans[i].time_step, self.humans[i].radius)
                ob.append(h)
